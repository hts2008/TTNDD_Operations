...
| W1-009  | READY       | B + E + F | Motion-site interaction system for the operational shell | `apps/web/src/app/globals.css`, layout components, shared UI components, high-traffic dashboard routes | Add motion tokens/classes inspired by premium motion-site patterns while preserving ERP density: route reveals, hover/press feedback, timeline/stepper transitions, progress animation, reduced-motion fallback, no workflow-blocking animation | Stitch/Figma design evidence if available; browser screenshots/video; reduced-motion capture                 |
...
| W2-003  | READY  | E + D | Normalize tickets list/detail and approval v2 UI                | `tickets/page.tsx`, `tickets/[id]/page.tsx`, `approvals/page.tsx`     | Tickets list/detail use API client; approval flow stepper shows current/pending/approved/rejected steps; comments and status transitions refresh from backend | Browser E2E J9                                             |
