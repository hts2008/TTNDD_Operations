...
| W2-004  | IMPLEMENTED | E + D | Normalize assets UI for file refs and lifecycle                 | `assets/page.tsx`, `components/ui/file-upload-finalizer.tsx`, file storage local signed URL APIs             | Asset photo upload uses upload-request -> object upload -> finalize -> READY fileRef, persists `photoFileRefIds`, and refreshes from API-backed inventory data | `tests/e2e/w2-004-assets-file-lifecycle.spec.ts` PASS 1/1; `docs/artifacts/w2-004-assets-file-lifecycle/summary.json` |
...
| W3-009  | PARTIAL | D + E     | Make File Storage upload reusable in UI                  | Shared upload component/hook and file-storage APIs                                                                 | Component handles signed URL upload, finalize, scan/READY status, error states; used by assets, scout, LMS, SOP                                                | `FileUploadFinalizer` implemented and proven in Assets via W2-004; Scout/LMS/SOP wiring remains |
