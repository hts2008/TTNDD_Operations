# 🏯 TTNDD_OPS — MASTER SPEC (PRODUCT-FIRST, RESEARCH-AUGMENTED, ALL-IN-ONE + CONTRACT PACK) v17.0 GO-LIVE COVERAGE REPORT + RELEASE DASHBOARD (HARD GATES) RELEASE
...
> **Phiên bản**: **17.0** — Ngày: **06/03/2026** (Asia/Ho_Chi_Minh) (Asia/Ho_Chi_Minh)
...
> **Mục tiêu của PHẦN VIII**: Roadmap phải “kéo theo” đầy đủ những gì MasterSpec đã chốt: **modules + Core App Engine + data + integration + UI/UX MMORPG + security + budget**.  
> **Nguyên tắc**: *Contract‑first* + *SSOT Sync* + *Safety P0* + *Budget cap*.

### 8.0 Cơ chế ĐỒNG BỘ & NHẤT QUÁN (Spec Sync System) — “Không còn chỗ lệch pha”
> **Giải pháp**: áp dụng **Spec Sync System** với 3 tầng: **(A) SSOT artifacts**, **(B) Sync Gates trong CI**, **(C) Change Request (CR)**.

#### 8.0.1 SSOT (Single Source of Truth) — 8 artifacts phải luôn đồng bộ
Mỗi thay đổi sản phẩm/kỹ thuật **BẮT BUỘC** cập nhật đủ **8 artifacts** sau (thiếu 1 = coi như chưa hoàn thành):
1) **PRD/Workflow** (luồng nghiệp vụ + acceptance criteria)  
2) **UI Contract** (Screen map + wireframe text + component mapping + tokens)  
3) **OpenAPI contract** (`/contracts/openapi/*.yaml`)  
4) **Event catalog** (`/contracts/events/catalog.json`)  
5) **DB schema & migrations** (`/contracts/db/migrations/*`)  
6) **Tests** (unit/integration/contract/e2e)  
7) **Roadmap row IDs** (Work Package table IDs + traceability)  
8) **ADR/Tech Stack Decisions** (version pinning + rationale)

#### 8.0.2 Sync Gate (CI) — “Không merge nếu lệch”
- Validate OpenAPI (schema + lint) + check breaking change.  
- Validate Event catalog (schema + compatibility rules).  
- Migration smoke test (apply trên DB test có RLS).  
- Contract tests (OpenAPI schema tests; optional Pact).  
- UI contract drift check (routes/components/tokens version).  
- DoD gate (auto-check: budget tags, safety gates, audit, RLS).

#### 8.0.3 Change Request (CR) — mẫu bắt buộc khi đổi yêu cầu
CR phải ghi rõ: Scope change → impacted modules → SSOT updates → migration plan → rollback plan → budget impact → safety impact.
...
### 8.2 Chi tiết Lộ trình (Go‑live “đủ bài”)

#### STORY-001: "XÂY DỰNG TTNDD_OPS v1.0 (GO‑LIVE READY)"

> **Vì sao không thể chỉ 3–4 phase**: ngoài module, còn phải “đủ” các mảng: **API Gateway, File Storage, Data Warehouse/ETL, UI Kit MMORPG, DR/PITR, Security Edge/WAF, CI/CD gates**.  
> Roadmap v13 trước đây có nhiều phần đã có, nhưng **thiếu Work Package explicit** cho các mảng trên nên nhìn “quá đơn giản”.

---

## PHASE 0 — FOUNDATION + PLATFORM OPS + UI KIT (Tuần 1–4)

| Work Package | Milestone | Tasks | Dev Notes (Implementation) |
|---|---|---|---|
| WP-0.1: Monorepo & Local Dev | M0.1: build/run local | T-001→T-010: Turborepo + pnpm workspace + docker-compose (postgres/redis); lint/format; env templates | Repo scaffold `/apps/api`, `/apps/web`, `/apps/worker`, `/packages/shared`, `/packages/ui`, `/packages/tokens` |
| WP-0.2: CI/CD Baseline | M0.2: deploy Cloud Run dev | T-011→T-022: Cloud Build pipeline; Artifact Registry; deploy web/api; env secrets; smoke health checks | Cloud Build triggers; Artifact Registry; Cloud Run services; Secret Manager bindings |
| WP-0.3: Core DB + RLS | M0.3: tenant isolation pass | T-023→T-038: core schema; RLS policies; seed; migration smoke test | PostgreSQL RLS + middleware `SET LOCAL app.org_id`; audit baseline |
| WP-0.4: Identity & Access | M0.4: multi-tenant auth OK | T-039→T-056: verify ID token; session JWT; RBAC/ABAC (CASL); scope model; parent/guardian link | Auth guard + CASL; parent portal restrictions |
| WP-0.5: API Gateway (Front Door) | M0.5: gateway routes OK | T-057→T-066: deploy API Gateway from OpenAPI; auth/quotas; staging & prod configs | OpenAPI SSOT → API Gateway deploy; env-based backend URL |
| WP-0.6: File Storage Service | M0.6: signed URL upload OK | T-067→T-078: private buckets; signed URL v4; file refs table; mime/size validation | GCS signed URL; store refs in `file.object_ref`; TTL downloads |
| WP-0.7: Event Bus + Outbox | M0.7: publish/consume OK | T-079→T-092: outbox table; publisher worker; Pub/Sub topics; DLQ; idempotent consumers | event envelope + unique(event_id); retry strategy |
| WP-0.8: Budget Guardrails ≤ 800k + Observability | M0.8: alerts + throttle | T-093→T-108: budgets 50/80/100/120; Pub/Sub alerts; Cloud Run max instances throttle; logging exclusions; dashboards | cost-impact tags; low-cost mode flags |
| WP-0.9: UI Foundation (MMORPG App Shell) | M0.9: HUD shell live | T-109→T-126: Design tokens build (Style Dictionary); AppShell + HUDTopBar/ActionBar; LeftRailNav; RightQuestPanel; routing skeleton | FE uses tokens CSS vars; reduced-motion; role gating |
| WP-0.10: Contract-first Framework | M0.10: gates running | T-127→T-145: OpenAPI lint; event catalog validator; migration smoke; UI contract drift check; DoD runner | CI blocks breaking changes |
| WP-0.11: Module Registry + Readiness Gate | M0.11: module-active ≠ empty | T-146→T-168: define release profiles; readiness manifest; generate “Getting Started” templates; module health endpoints | Create `/contracts/release/module-readiness.yaml`; implement `GET /system/module-health`; FE hides ACTIVE modules failing readiness |

---

## PHASE 1 — CORE APP ENGINE + ADMIN (Tuần 5–8)

| Work Package | Milestone | Tasks | Dev Notes (Implementation) |
|---|---|---|---|
| WP-1.1: Module 10 Org Config/IAM | M1.1: org bootstrap | T-146→T-170: org CRUD; roles/scopes; module toggles; audit viewer; feature flags | contract pack M10; RLS + audit diff |
| WP-1.2: Module 1 HRM | M1.2: member lifecycle | T-171→T-210: roster; profile; guardian; emergency contacts; transfer; org chart | contract pack M1; parent view read-only |
| WP-1.3: Diagram Pack (SSOT) | M1.3: diagrams complete | T-211→T-230: architecture diagram; data flow; event flow; security model; module boundaries | export as Mermaid blocks; link from PRD/Engineering |
| WP-1.4: Data Import/Mapping v1 | M1.4: import safe | T-231→T-248: CSV templates; validators; mapping rules; dry-run; audit imports | avoid PII leaks; rollback import batches |

---

## PHASE 2 — SCOUT CORE + REWARDS + SAFETY (Tuần 9–14)

| Work Package | Milestone | Tasks | Dev Notes (Implementation) |
|---|---|---|---|
| WP-2.1: Module 9 Reward Engine | M2.1: exp/badges stable | T-249→T-292: ledger immutable; caps; anti-abuse; leaderboard snapshots; shop/redemption | subscribe events; dedupe by event_id |
| WP-2.2: Module 8 Skillbook | M2.2: skill tree v1 | T-293→T-340: program versions; skill tree; evidence upload; moderation queue; verify flow; emits reward events | state machine strict; evidence private |
| WP-2.3: Module 8 Sessions | M2.3: attendance OK | T-341→T-372: session CRUD; attendance bulk; SPICES tags required; rollup by party(4–8) | idempotent bulk endpoints |
| WP-2.4: Module 8 Events/Camps | M2.4: camp workflow OK | T-373→T-418: registration/waitlist; consent e-sign; HIRARC; 2-adult gate; camp pack export | safety gates mandatory |
| WP-2.5: Incident Reporting (P0) | M2.5: sensitive reports | T-419→T-452: restricted RBAC; redaction; encryption fields; escalation; retention | separate scopes; audit all access |
| WP-2.6: Notifications v1 | M2.6: in-app inbox | T-453→T-476: notif center; templates; quiet hours; weekly parent summary | Zalo optional later |

---

## PHASE 3 — OPS MODULES (Tuần 15–20)

| Work Package | Milestone | Tasks | Dev Notes (Implementation) |
|---|---|---|---|
| WP-3.1: Module 2 Projects/Planning | M3.1: plan→project | T-477→T-536: plan composer; approval; generator; kanban; gantt flag; reminders | gantt behind kill-switch |
| WP-3.2: Module 3 Ticket/Approval | M3.2: approvals patterns | T-537→T-580: sequential/parallel/conditional/multi-level; attachments | approval templates + SLA |
| WP-3.3: Module 4 Finance | M3.3: fees/ledger | T-581→T-640: fee engine; partial payments; campership; reports | immutable ledger; audit approvals |
| WP-3.4: Module 5 Assets | M3.4: loans/inventory | T-641→T-700: inventory; loan workflow; QR; kits; uniform issue | state machine |
| WP-3.5: Module 6 Process/SOP | M3.5: SOP library | T-701→T-740: workflow builder v1; SOP versioning; safeguards | timeouts + max depth |
| WP-3.6: Zalo OA Integration | M3.6: Zalo live (opt-in) | T-741→T-770: OA setup; delivery logs; retries; feature flag | can disable in low-cost mode |

---

## PHASE 4 — LMS + ENGAGEMENT (Tuần 21–24)

| Work Package | Milestone | Tasks | Dev Notes (Implementation) |
|---|---|---|---|
| WP-4.1: Module 7 LMS Core | M4.1: courses/quizzes | T-771→T-830: course/lesson builder; quiz engine; grading; mentor queue | SPICES tags required |
| WP-4.2: Offline Packs (Camp Mode) | M4.2: offline usable | T-831→T-860: offline pack builder; SW caching; sync | bandwidth guarded |
| WP-4.3: Arena/Battle (optional) | M4.3: arena gated | T-861→T-900: socket rooms; anti-cheat; caps | kill-switch HIGH cost |

---

## PHASE 5 — DATA WAREHOUSE + REPORTING (Tuần 25–28)

| Work Package | Milestone | Tasks | Dev Notes (Implementation) |
|---|---|---|---|
| WP-5.1: Read Models (OLTP) | M5.1: aggregates stable | T-901→T-930: materialized views; caching; consistency checks | keep OLTP fast |
| WP-5.2: BigQuery DWH + ETL | M5.2: events→BQ live | T-931→T-970: dataset/tables; Pub/Sub→BQ (Dataflow template); schema mapping; backfill | streaming ingestion + partitions |
| WP-5.3: Analytics Dashboards | M5.3: org analytics | T-971→T-1010: SPICES coverage; progress; finance; attendance; exports | avoid shame ranking |

---

## PHASE 6 — HARDENING + GO‑LIVE (Tuần 29–32)

| Work Package | Milestone | Tasks | Dev Notes (Implementation) |
|---|---|---|---|
| WP-6.1: Security Edge + WAF | M6.1: WAF baseline | T-1011→T-1040: Cloud Armor rate limiting; headers; threat checks | protect API edge |
| WP-6.2: DR/Backups/PITR | M6.2: restore drill pass | T-1041→T-1060: enable PITR; backup retention; restore drills; runbook | DR readiness |
| WP-6.3: Performance & QA Gates | M6.3: e2e pass | T-1061→T-1090: k6 load; Playwright; contract tests; migration tests | “no contract break” |
| WP-6.4: Pilot & Go‑live | M6.4: rollout done | T-1091→T-1120: pilot org; training; data import; monitoring; budget drill | rollback plan |
| WP-6.5: Go‑Live Hard Gates (Release Control) | M6.5: release blocked if empty | T-1121→T-1155: enforce readiness manifest; run Playwright gate pack; post-deploy canary; Cloud Deploy approval for prod | CI: `validate:readiness`, `e2e:module-gates`; post-deploy `canary` hits critical endpoints; approvals via Cloud Deploy |

---

### 8.3 Coverage Checklist (đối chiếu với yêu cầu tổng)
- **Core App Engine**: WP-0.3/0.4/0.7 + WP-1.1  
- **API Gateway/API**: WP-0.5 + Contract gates  
- **File Storage**: WP-0.6  
- **Event-Driven Architecture**: WP-0.7 + WP-5.2  
- **UI/UX MMORPG**: WP-0.9 + Screen map (PHẦN V)  
- **Data warehouse / data mapping**: WP-5.2/5.3  
- **Security**: WP-0.4/2.5/6.1  
- **Go-live**: WP-6.4 + budget drill
...
### 8.4 “MODULE NOT EMPTY” GUARANTEE — Ma trận ĐỦ DÙNG (Playable MVP) cho từng module

> Vấn đề bạn phản ánh (“có module nhưng vào không có gì để xài”) thường xảy ra khi chỉ dựng **navigation + shell** mà chưa có:
> **(a)** dữ liệu seed/demo, **(b)** workflow end‑to‑end, **(c)** UI screens, **(d)** APIs & schema, **(e)** quyền/nhật ký/audit.  
> Vì vậy, mỗi module phải đạt “Playable MVP” trước khi coi là DONE.

#### 8.4.1 Tiêu chuẩn “Playable MVP” (áp dụng mọi module)
Một module được coi là **đã chạy được** khi đạt đủ:
1) **UI Screens P0** (tối thiểu 2–5 màn hình) + empty/loading/error states  
2) **Workflow end‑to‑end** (tạo → xử lý → duyệt/hoàn tất → xem báo cáo)  
3) **OpenAPI contract** đầy đủ (request/response/error codes)  
4) **DB migrations + RLS** (org isolation) + indexes tối thiểu  
5) **Domain events** (nếu có) + idempotent consumers  
6) **Seed data pack** (demo fixtures) để “vào là thấy có cái để dùng”  
7) **Tests**: unit + integration + 1 e2e smoke cho module  
8) **Audit** cho thao tác nhạy cảm + cost tags + safety gates

#### 8.4.2 Module Readiness Matrix (P0 go‑live)
| Module | “Playable MVP” workflow (P0) | Screens tối thiểu (routes) | APIs tối thiểu (tag) | Data / Tables tối thiểu | Roadmap WP |
|---|---|---|---|---|---|
| M10 Org/IAM | tạo Org → tạo Unit/Ngành/Đội → gán role/scope → bật module flag | `/settings/org`, `/settings/iam`, `/settings/modules`, `/settings/audit` | Org/IAM/Config | `core.organizations`, `org.units`, `iam.*`, `audit.*` | WP‑1.1 |
| M1 HRM | tạo hồ sơ → gắn phụ huynh → phân ngành/đội → chuyển ngành (handover trigger) | `/guild/roster`, `/character/:id`, `/guild/org-chart`, `/guild/transfers` | HRM/Org | `hrm.person_profile`, `hrm.membership`, `hrm.guardian_link`, `hrm.transfer_case` | WP‑1.2 |
| M2 Project | tạo kế hoạch mẫu → submit/approve → auto sinh project tree → kanban update | `/quest`, `/quest/plans/new`, `/quest/projects/:id/board`, `/quest/projects/:id/tree` | PM | `pm.plan_*`, `pm.project`, `pm.work_item` | WP‑3.1 |
| M3 Ticket | tạo request + upload → route → approve/reject → close | `/inbox/tickets`, `/inbox/tickets/new`, `/inbox/tickets/:id` | Ticket/File | `ticket.case`, `ticket.approval_*`, `file.object_ref` | WP‑3.2 |
| M4 Finance | tạo phí đoàn viên → thu (partial) → ledger → báo cáo thu/chi | `/guild/treasury`, `/guild/treasury/fees`, `/guild/treasury/ledger` | Finance | `fin.ledger`, `fin.fees`, `fin.payments` | WP‑3.3 |
| M5 Assets | tạo tài sản → request mượn → approve → check‑out → return | `/guild/armory`, `/guild/armory/loans` | Assets | `asset.items`, `asset.loans`, `asset.history` | WP‑3.4 |
| M6 Process | tạo SOP → publish → workflow trigger từ event → log run | `/guild/process/sop`, `/guild/process/workflows`, `/guild/process/builder/:id` | Process | `proc.workflow_def`, `proc.workflow_run`, `proc.sop_*` | WP‑3.5 |
| M7 LMS | course + lesson → quiz attempt → pass → emit event | `/academy`, `/academy/courses`, `/academy/quizzes/:id` | LMS | `lms.course`, `lms.lesson`, `lms.quiz`, `lms.attempt` | WP‑4.1 |
| M8 Scout | skill tree → upload evidence → submit → verify → award → handover case | `/character/:id/skills`, `/guild/scout/verify`, `/guild/scout/handover/:id` | Scout/File | `scout.skill_*`, `scout.progress`, `scout.handover_*` | WP‑2.2/2.4 |
| M9 Rewards | consume event → exp ledger → badge award → leaderboard | `/character/:id/inventory`, `/guild/rewards/leaderboard` | Reward | `reward.exp_ledger`, `reward.badge_*`, `reward.leaderboard` | WP‑2.1 |

> **Note:** Các baseline tính năng tham chiếu (để “đúng chuẩn hệ thống”):
> - OpenProject “work packages”/time tracking/gantt cho module PM. citeturn0search8turn0search0turn0search20  
> - OrangeHRM modules (Leave/Discipline/Org Chart…) cho HRM. citeturn0search5turn0search1  
> - ERPNext Cost Center & Budgeting cho Finance. citeturn0search2turn0search18  
> - Snipe‑IT checkout/checkin + notifications cho Assets. citeturn0search15turn0search11  
> - Moodle offline features cho Camp Mode/LMS. citeturn1search1turn1search10  
> - Zammad triggers/workflows làm mẫu automation cho Ticket/Approvals. citeturn1search0turn1search4  

---

### 8.5 Seed Data Pack & Demo Scenarios (bắt buộc để “vào là có cái dùng”)

#### 8.5.1 Seed Data Pack (P0)
- **Org demo**: 1 Liên đoàn, 3 Ngành (Đồng/Thiếu/Thanh), 2 Đội/Ngành, 1 Nhóm/Đội.
- **Members demo**: 2 Trưởng, 12 Đoàn sinh, 6 phụ huynh (guardian links).
- **Scout program demo**: 1 version, 3 ranks, mỗi rank 2 domains, mỗi domain ≥ 6 skills, mỗi skill ≥ 4 criteria.
- **PM templates demo**: 2 plan templates (Sinh hoạt tháng, Trại cuối tuần) + OKR mapping.
- **Finance demo**: 1 fee plan (đoàn phí) + 1 camp fee (partial payments) + 1 sponsor + 1 campership.
- **Assets demo**: 10 items + 2 kits + 3 uniforms sizes.
- **LMS demo**: 1 course “Giáo lý Cao Đài nhập môn” + 1 quiz.
- **Rewards demo**: 5 exp rules + 10 badges + daily cap rules.

#### 8.5.2 Demo Scenarios (E2E) — validate “module not empty”
1) **Onboard đoàn sinh**: HRM create → link guardian → assign unit → show Character Sheet.
2) **Plan to Questline**: tạo kế hoạch → approve → auto project tree → assign tasks → reminders.
3) **Camp readiness**: create event → consent e‑sign → HIRARC gate → assets kit loan → camp fee collection.
4) **Skill progression**: evidence upload → verify → reward XP/badge → leaderboard updates.
5) **Incident report**: create incident → restricted view → escalation + retention.
6) **Offline pack**: download pack → mark attendance/checklist offline → sync back.
7) **Analytics**: stream events → BigQuery → dashboards (SPICES coverage & progress).

> BigQuery streaming pipeline tham chiếu: Dataflow template Pub/Sub→BigQuery. citeturn1search3  
> DR go-live tham chiếu: Cloud SQL PITR enable + perform restore drill. citeturn1search2turn1search6  

---

### 8.6 Roadmap chỉnh để “build đúng thứ mô tả” — bổ sung Deliverables per WP (không chỉ skeleton)

> Dưới đây là *bổ sung* quy định Deliverables bắt buộc cho các WP module (để tránh tình trạng “có menu module nhưng trống”).

#### WP‑1.2 (HRM) Deliverables (P0)
- **Screens**: Roster, Character Sheet, Org chart, Transfer wizard.
- **APIs**: CRUD members, guardian link, membership assign, transfer case.
- **Data**: emergency contacts + medical profile baseline (read-only if needed).
- **Workflow**: transfer triggers `scout.handover_case` creation when branch change.
- **Tests**: under‑18 requires guardian; transfer audit; RLS isolation.

#### WP‑2.2/2.4 (Scout) Deliverables (P0)
- **Skill tree** must show locked/unlocked/in-progress, and a working “submit evidence”.
- **Verification queue** must approve/reject with reasons and emit reward events.
- **Handover** must generate a summary package and enforce “2-adult rule” for camp events.

#### WP‑3.1 (PM) Deliverables (P0)
- **Plan composer** uses structured template; approve creates project tree.
- **Kanban** must allow status change; reminders.
- **Gantt** is behind kill-switch (HIGH cost) but tree/list must work.

#### WP‑5.2 (DWH) Deliverables (P0)
- Must ingest domain events into BigQuery with schema mapping + partitions.
- Must run backfill for last 7 days.
...
### 8.7 GO‑LIVE HARD GATES — “Không thể go‑live khi module còn trống”

> **Định nghĩa “trống”**: module được bật **ACTIVE** (hiển thị trong World Map) nhưng thiếu **workflow end‑to‑end** hoặc thiếu **data/templates tối thiểu** để thao tác.  
> **Nguyên tắc**: *Không cho phép “module chỉ có menu”*. Nếu chưa đủ → chỉ được ở trạng thái **COMING_SOON** (ẩn/khóa thao tác).

#### 8.7.1 Release Profiles (để kiểm soát phạm vi go‑live)
- **PROFILE‑CORE (Go‑live v1)**: M10 Org/IAM, M1 HRM, M8 Scout, M9 Rewards, Notifications, File, API Gateway.
- **PROFILE‑OPS**: + M2 Project, M3 Ticket, M4 Finance, M5 Assets, M6 Process.
- **PROFILE‑FULL**: + M7 LMS + Offline packs + Arena (gated).

> **Rule**: chỉ module nằm trong profile được phép **ACTIVE**. Module ngoài profile phải COMING_SOON (không tính “trống”).

#### 8.7.2 Go‑Live Gate Manifest (SSOT)
Tạo 1 file SSOT để CI kiểm tra “module active ≠ empty”:

**`/contracts/release/module-readiness.yaml`** (ví dụ)
```yaml
...
modules:
  HRM:
    ...
    required_seed:
      ...
      - members_minimal
    required_e2e:
      - e2e/hrm/onboard-and-transfer.spec.ts
  SCOUT:
    active: true
    required_routes:
      - /character/:id/skills
      - /guild/scout/verify
    required_openapi_tags: [Scout, File]
    required_seed:
      - scout_program_minimal
      - skills_minimal
    required_e2e:
      - e2e/scout/skill-submit-verify-award.spec.ts
```

#### 8.7.3 Hard Checks (CI) — fail = block merge/release
**Static checks**
- `validate:openapi` (lint + schema)
- `validate:events` (catalog)
- `validate:migrations` (apply on test DB, RLS on)
- `validate:ui-contract` (routes/components/tokens)
- `validate:readiness`:
  - verify required routes exist in UI contract  
  - verify required OpenAPI tags exist  
  - verify seed scripts exist and referenced by manifest  
  - verify module is not marked ACTIVE without readiness passing

**Dynamic checks**
- `e2e:module-gates` (Playwright):
  - Run only tests required by manifest (tag-based)  
  - Must pass on staging before prod promote  
  - Retry policy + trace on first retry

> Playwright CI + retries/trace best practice:  
...
#### 8.7.4 Post‑deploy Canary (hard gate)
Sau deploy staging/prod, chạy canary:
- `GET /healthz` (api/web)
- `GET /system/module-health` → trả về `{ module: { active, ready, reason[] } }`
- `GET /system/seed-health` → kiểm tra templates tối thiểu đã có
- Nếu canary fail → auto rollback (hoặc block promote).

---

### 8.8 E2E Playwright Gate Pack — danh sách test “đủ dùng” theo module

> **Quy tắc**: mỗi module ACTIVE phải có ≥ 1 test E2E “happy path” + 1 test “permission boundary” (parent/leader scopes).

| Module | E2E Test (happy path) | E2E Test (security boundary) | Tags |
|---|---|---|---|
| M10 Org/IAM | `e2e/org/bootstrap-org.spec.ts` | `e2e/org/rbac-scope.spec.ts` | `@module:org @gate` |
| M1 HRM | `e2e/hrm/onboard-and-transfer.spec.ts` | `e2e/hrm/parent-readonly.spec.ts` | `@module:hrm @gate` |
| M8 Scout | `e2e/scout/skill-submit-verify-award.spec.ts` | `e2e/scout/evidence-private.spec.ts` | `@module:scout @gate` |
| M9 Rewards | `e2e/reward/award-ledger-leaderboard.spec.ts` | `e2e/reward/anti-abuse-cap.spec.ts` | `@module:reward @gate` |
| M2 Project | `e2e/pm/plan-to-project-kanban.spec.ts` | `e2e/pm/rbac-project-access.spec.ts` | `@module:pm @gate` |
| M3 Ticket | `e2e/ticket/request-approve-close.spec.ts` | `e2e/ticket/permission-scope.spec.ts` | `@module:ticket @gate` |
| M4 Finance | `e2e/fin/fee-partial-ledger-report.spec.ts` | `e2e/fin/approve-only-admin.spec.ts` | `@module:fin @gate` |
| M5 Assets | `e2e/assets/loan-approve-return.spec.ts` | `e2e/assets/scope-unit-only.spec.ts` | `@module:assets @gate` |
| M6 Process | `e2e/proc/sop-publish-workflow-run.spec.ts` | `e2e/proc/workflow-safeguards.spec.ts` | `@module:proc @gate` |
| M7 LMS (optional) | `e2e/lms/course-lesson-quiz-pass.spec.ts` | `e2e/lms/mentor-scope.spec.ts` | `@module:lms @gate` |

**Playwright config baseline (gate mode)**
- Retries = 2 (CI only), trace = `on-first-retry`, video = `retain-on-failure`.
- Shard by module tags nếu chạy lâu.

---

### 8.9 “Không trống” ở Production — Getting Started bắt buộc (không dùng demo seed)
Để prod không cần seed demo mà vẫn “vào là có cái dùng”, mọi module ACTIVE phải có:
- **Getting Started Wizard** (tạo cấu trúc tối thiểu):
  - Org → tạo 3 ngành + 1 đội mẫu
  - HRM → import CSV tối thiểu + link phụ huynh
  - Scout → tạo program version + import skill set
  - PM → chọn plan template
- **Default templates** (không phải dữ liệu giả):
  - plan templates, consent templates, SOP templates, badge rules starter pack.
- **Module Health**:
  - module ready = true khi có ≥ 1 template + ≥ 1 entity instance + workflow runnable.
...
### 8.10 Technical Reference Index (để roadmap link sang phần kỹ thuật)

> Mục tiêu: đảm bảo **Dev Notes (Implementation)** không “nói chung chung” mà **trỏ đúng** phần kỹ thuật trong MasterSpec.

| Nhóm kỹ thuật | Anchor | Phần mô tả |
|---|---|---|
| Tech Stack | [REF-TECHSTACK](#REF-TECHSTACK) | FE/BE/DB/Event/CI/CD pinned stack |
| AI Delivery Model | [REF-AIDELIVERY](#REF-AIDELIVERY) | Vai trò luân phiên + Sync Gates |
| ADR Registry | [REF-ADR](#REF-ADR) | Quyết định công nghệ + version pinning |
| Core App Engine | [REF-COREENGINE](#REF-COREENGINE) | Module map + bounded context + schemas |
| Event-Driven | [REF-EVENT](#REF-EVENT) | Outbox + Pub/Sub + idempotency |
| Security & Privacy | [REF-SECURITY](#REF-SECURITY) | PII, audit, child safety gates |
| Identity & Access | [REF-IAM](#REF-IAM) | Multi-tenant, scopes, parent portal |
| File Storage | [REF-FILE](#REF-FILE) | Signed URL, private bucket, scanning limits |
| Data Warehouse | [REF-DWH](#REF-DWH) | BigQuery schema mapping + ingestion |
| UI/UX MMORPG | [REF-UX](#REF-UX) | HUD system + game components |
| UI Screen Map | [REF-SCREENMAP](#REF-SCREENMAP) | Routes + component mapping |
| Design Tokens | [REF-TOKENS](#REF-TOKENS) | DTCG JSON + build output |
| Budget Guardrails | [REF-BUDGET](#REF-BUDGET) | 800k cap + kill-switch + low-cost mode |

---

### 8.11 Module Capability Map (Feature/Workflow → Screens → API → DB → Events → Tests)

> **Tại sao cần**: “Readiness” chỉ chứng minh module *không trống*.  
> “Capability” chứng minh module *đúng chức năng, đúng mô tả* và có coverage kỹ thuật đầy đủ để vận hành.

#### 8.11.1 Capability Manifest (SSOT) — bắt buộc để ACTIVE
Tạo thêm 1 SSOT file cho capability coverage:

**`/contracts/release/module-capabilities.yaml`**
```yaml
modules:
  HRM:
    epics:
      - id: HRM-E1
        ...
        workflow: create->assign unit->view character sheet
        screens: [/guild/roster, /character/:id]
        openapi_tags: [HRM, Org]
        tables: [hrm.person_profile, hrm.membership, hrm.guardian_link]
        events_out: [hrm.member_joined]
        tests:
          unit: [MemberLifecycleService]
          integration: [hrm.members.spec.ts]
          e2e: [e2e/hrm/onboard-and-transfer.spec.ts]
  SCOUT:
    epics:
      - id: SCOUT-E2
        name: Evidence Verification
        workflow: upload evidence->submit->approve/reject->award
        screens: [/character/:id/skills/:skillId, /guild/scout/verify]
        openapi_tags: [Scout, File, Reward]
        tables: [scout.skill, scout.progress, scout.evidence, reward.exp_ledger]
        events_out: [scout.skill.verified, reward.exp_earned]
        tests:
          e2e: [e2e/scout/skill-submit-verify-award.spec.ts]
```

#### 8.11.2 Hard Gate bổ sung
- `validate:capabilities` (static):
  - Epic phải có đủ: workflow + ≥1 screen + ≥1 API tag + tables + ≥1 e2e
- `e2e:capabilities` (dynamic):
  - chạy đúng tập e2e theo capability manifest (tag-based)
- Nếu module **ACTIVE** mà thiếu epic coverage → **block promote**.

---

### 8.12 Module Development Track — Roadmap riêng để “build đúng tính năng/mô tả”

> Track này **không thay** platform core roadmap. Nó *siết* ở tầng module: mỗi Epic = 1 capability, và phải link tới kỹ thuật (API/DB/Event/UI/Sec/DWH).

#### 8.12.1 Format roadmap row (chuẩn)
- **Epic ID**: `M{module}-E{n}`
- **Outcome**: người dùng làm được gì
- **Deliverables**: Screens + OpenAPI + DB/RLS + Events + Tests + Seed/Templates
- **Dev Notes**: luôn có links tới technical anchors: `REF-*`

---

## 8.12.2 Roadmap — Module 10 (Org & IAM)
| Epic ID | Feature/Function (mô tả) | Workflow P0 | Deliverables (must-have) | Dev Notes (Implementation — link kỹ thuật) |
|---|---|---|---|---|
| M10-E1 | Org bootstrap & structure (Ngành/Đội/Unit) | create org → create units → assign unit managers | Screens: `/settings/org`, `/settings/org-structure` · API tags: Org/Config · DB: `core.organizations`, `org.units` · Tests: e2e org bootstrap | Refs: [REF-IAM](#REF-IAM), [REF-COREENGINE](#REF-COREENGINE), [REF-TECHSTACK](#REF-TECHSTACK) |
| M10-E2 | RBAC/ABAC scopes + role assignment | grant role → scope unit/branch → enforce in UI | API: IAM · DB: `iam.role_scope` + RLS policies · Audit logs | Refs: [REF-IAM](#REF-IAM), [REF-SECURITY](#REF-SECURITY), [REF-COREENGINE](#REF-COREENGINE) |
| M10-E3 | Module flags + low-cost switches | toggle modules → UI re-renders → gates apply | DB: `cfg.module_flags` · API: Config · FE: world map gating | Refs: [REF-BUDGET](#REF-BUDGET), [REF-UX](#REF-UX) |

---

## 8.12.3 Roadmap — Module 1 (HRM)
| Epic ID | Feature/Function (mô tả) | Workflow P0 | Deliverables (must-have) | Dev Notes (Implementation — link kỹ thuật) |
|---|---|---|---|---|
| M1-E1 | Roster + Member Profile (Character Sheet base) | create member → view profile → edit fields | Screens: `/guild/roster`, `/character/:id` · API: HRM · DB: `hrm.person_profile` · RLS + audit | Refs: [REF-UX](#REF-UX), [REF-SCREENMAP](#REF-SCREENMAP), [REF-COREENGINE](#REF-COREENGINE), [REF-SECURITY](#REF-SECURITY) |
| M1-E2 | Guardian link + Parent portal (read-only) | link guardian → parent sees child progress | API: HRM/Scout/Reward read models · DB: `hrm.guardian_link` · Permission tests | Refs: [REF-IAM](#REF-IAM), [REF-SECURITY](#REF-SECURITY) |
| M1-E3 | Org chart assignments | assign unit/branch → org chart renders | Screen: `/guild/org-chart` · DB: `org.unit_membership` · cache read model | Refs: [REF-COREENGINE](#REF-COREENGINE), [REF-DWH](#REF-DWH) |
| M1-E4 | Transfers + handover trigger | initiate transfer → approve → scout handover created | Workflow + Event: `hrm.member_transferred` → `scout.handover_case.created` | Refs: [REF-EVENT](#REF-EVENT), [REF-COREENGINE](#REF-COREENGINE) |

---

## 8.12.4 Roadmap — Module 2 (Project/Planning)
| Epic ID | Feature/Function (mô tả) | Workflow P0 | Deliverables (must-have) | Dev Notes (Implementation — link kỹ thuật) |
|---|---|---|---|---|
| M2-E1 | Plan Composer (template structured) | compose plan → submit → approve/reject | Screens: `/quest/plans/new` · API: PM · DB: `pm.plans` · Approval states | Refs: [REF-UX](#REF-UX), [REF-TECHSTACK](#REF-TECHSTACK) |
| M2-E2 | Plan → Project generator (OKR/Task tree) | approve plan → auto create project tree | DB: `pm.projects/work_items` · Event: `pm.plan.approved` | Refs: [REF-EVENT](#REF-EVENT), [REF-COREENGINE](#REF-COREENGINE) |
| M2-E3 | Kanban + progress | move task status → progress updates | Screen: `/quest/projects/:id/board` · API: PM · Tests: state machine transitions | Refs: [REF-UX](#REF-UX), [REF-SECURITY](#REF-SECURITY) |
| M2-E4 | Gantt (kill-switch) | view gantt (optional) | behind feature flag; compute on-demand | Refs: [REF-BUDGET](#REF-BUDGET) |

---

## 8.12.5 Roadmap — Module 3 (Ticket/Approval)
| Epic ID | Feature/Function (mô tả) | Workflow P0 | Deliverables (must-have) | Dev Notes (Implementation — link kỹ thuật) |
|---|---|---|---|---|
| M3-E1 | Ticket create + attachment | create → upload → route | Screen `/inbox/tickets/new` · File signed URL upload · DB: `ticket.case`, `file.object_ref` | Refs: [REF-FILE](#REF-FILE), [REF-SECURITY](#REF-SECURITY) |
| M3-E2 | Approval patterns (sequential/parallel/conditional) | approve/reject → close | DB: `ticket.approval_*` · Events: `ticket.approved` | Refs: [REF-EVENT](#REF-EVENT), [REF-COREENGINE](#REF-COREENGINE) |

---

## 8.12.6 Roadmap — Module 4 (Finance)
| Epic ID | Feature/Function (mô tả) | Workflow P0 | Deliverables (must-have) | Dev Notes (Implementation — link kỹ thuật) |
|---|---|---|---|---|
| M4-E1 | Fees & partial payments | generate fee → pay partial → ledger updated | DB: `fin.fees`, `fin.payments`, immutable ledger | Refs: [REF-COREENGINE](#REF-COREENGINE), [REF-SECURITY](#REF-SECURITY) |
| M4-E2 | Budget & cost centers (P1) | define cost centers → budget vs actual | DWH mapping for reporting | Refs: [REF-DWH](#REF-DWH) |

---

## 8.12.7 Roadmap — Module 5 (Assets)
| Epic ID | Feature/Function (mô tả) | Workflow P0 | Deliverables (must-have) | Dev Notes (Implementation — link kỹ thuật) |
|---|---|---|---|---|
| M5-E1 | Inventory CRUD | create assets → assign category | DB: `asset.items` · Screens `/guild/armory` | Refs: [REF-COREENGINE](#REF-COREENGINE) |
| M5-E2 | Loan workflow | request → approve → checkout → return | events `asset.loan.*` + audit | Refs: [REF-EVENT](#REF-EVENT), [REF-SECURITY](#REF-SECURITY) |

---

## 8.12.8 Roadmap — Module 6 (Process/SOP)
| Epic ID | Feature/Function (mô tả) | Workflow P0 | Deliverables (must-have) | Dev Notes (Implementation — link kỹ thuật) |
|---|---|---|---|---|
| M6-E1 | SOP library + versioning | create sop → publish → view | DB: `proc.sop_*` · UI docs reader | Refs: [REF-UX](#REF-UX) |
| M6-E2 | Workflow builder + executor | build workflow → execute on event | consumes event catalog; safeguards | Refs: [REF-EVENT](#REF-EVENT), [REF-SECURITY](#REF-SECURITY) |

---

## 8.12.9 Roadmap — Module 7 (LMS)
| Epic ID | Feature/Function (mô tả) | Workflow P0 | Deliverables (must-have) | Dev Notes (Implementation — link kỹ thuật) |
|---|---|---|---|---|
| M7-E1 | Course/Lesson player | enroll → play lesson → complete | File streaming gated; progress tracking | Refs: [REF-FILE](#REF-FILE), [REF-BUDGET](#REF-BUDGET) |
| M7-E2 | Quiz + grading | attempt → pass → emit event | event → rewards/scout unlock | Refs: [REF-EVENT](#REF-EVENT) |
| M7-E3 | Offline packs (optional) | download pack → offline record → sync | DWH mapping for offline sync audit | Refs: [REF-DWH](#REF-DWH), [REF-SECURITY](#REF-SECURITY) |

---

## 8.12.10 Roadmap — Module 8 (Scout Core)
| Epic ID | Feature/Function (mô tả) | Workflow P0 | Deliverables (must-have) | Dev Notes (Implementation — link kỹ thuật) |
|---|---|---|---|---|
| M8-E1 | Skill Tree navigation | view tree → see locks/progress | `C-SkillTree` + APIs Scout | Refs: [REF-SCREENMAP](#REF-SCREENMAP), [REF-UX](#REF-UX) |
| M8-E2 | Evidence submit (uploads) | upload → submit | signed URL upload + moderation metadata | Refs: [REF-FILE](#REF-FILE), [REF-SECURITY](#REF-SECURITY) |
| M8-E3 | Verification queue | approve/reject → audit | emits `scout.skill.verified` | Refs: [REF-EVENT](#REF-EVENT), [REF-COREENGINE](#REF-COREENGINE) |
| M8-E4 | Rank achieved + handover | rank achieved → handover case | state machines + events | Refs: [REF-EVENT](#REF-EVENT), [REF-IAM](#REF-IAM) |
| M8-E5 | Sessions/attendance | mark attendance → progress updates | idempotent bulk endpoints | Refs: [REF-TECHSTACK](#REF-TECHSTACK), [REF-SECURITY](#REF-SECURITY) |

---

## 8.12.11 Roadmap — Module 9 (Rewards)
| Epic ID | Feature/Function (mô tả) | Workflow P0 | Deliverables (must-have) | Dev Notes (Implementation — link kỹ thuật) |
|---|---|---|---|---|
| M9-E1 | EXP engine (consume events) | consume event → exp txns | outbox/publisher + dedupe | Refs: [REF-EVENT](#REF-EVENT), [REF-COREENGINE](#REF-COREENGINE) |
| M9-E2 | Badges & awards | award badge → inventory updates | UI inventory; ledger immutable | Refs: [REF-UX](#REF-UX), [REF-SECURITY](#REF-SECURITY) |
| M9-E3 | Leaderboard | compute → display (anti-shame) | caching + caps + privacy | Refs: [REF-BUDGET](#REF-BUDGET), [REF-SECURITY](#REF-SECURITY) |

---

### 8.13 Consistency & Sync Checklist (để không bị “update lệch pha”)

> Mỗi Epic merge PR phải cập nhật **đúng phần** theo link trong Dev Notes.

**Checklist per Epic**
- [ ] PRD/Workflow updated (what/why)  
- [ ] UI Contract updated (routes/components/tokens usage) → [REF-SCREENMAP](#REF-SCREENMAP), [REF-TOKENS](#REF-TOKENS)  
- [ ] OpenAPI updated (tag+schemas) → [REF-TECHSTACK](#REF-TECHSTACK)  
- [ ] DB migrations + RLS policies updated → [REF-COREENGINE](#REF-COREENGINE)  
- [ ] Event catalog updated (if emits/consumes) → [REF-EVENT](#REF-EVENT)  
- [ ] Tests added (unit+integration+e2e) → [REF-AIDELIVERY](#REF-AIDELIVERY)  
- [ ] DWH mapping updated if affects metrics → [REF-DWH](#REF-DWH)  
- [ ] Security review + audit logs (PII/child safety) → [REF-SECURITY](#REF-SECURITY)  
- [ ] Budget impact annotated + kill-switch tag → [REF-BUDGET](#REF-BUDGET)



---

### 8.14 Coverage Report Automation (CI) — Báo cáo đồng bộ “module/capability coverage” theo release

> **Mục tiêu**: Khi bạn mở hệ thống, module không được “trống”.  
> V15/V16 chặn bằng readiness/capability manifests. V17 bổ sung 1 lớp **Coverage Report** để:
> - Tổng hợp **độ phủ**: PRD ↔ UI ↔ OpenAPI ↔ DB ↔ Events ↔ Tests ↔ Seed/Templates
> - Xuất **báo cáo máy đọc được** (JSON) và báo cáo review (MD)
> - Làm **hard gate**: coverage fail → không promote staging/prod.

#### 8.14.1 Outputs (SSOT của release, lưu trong repo & artifacts)
Trong repo:
- `/contracts/release/module-readiness.yaml`
- `/contracts/release/module-capabilities.yaml`

Build outputs (generated):
- `/contracts/release/out/coverage-report.json`
- `/contracts/release/out/coverage-report.md`
- `/contracts/release/out/playwright-report/` (HTML report, zip)

#### 8.14.2 Coverage rules (định nghĩa “đủ dùng” theo capability)
Một Epic (capability) được coi là **PASS** khi:
- Có workflow mô tả rõ  
- Có ≥ 1 screen route (trong UI contract)  
- Có ≥ 1 OpenAPI tag + endpoint tương ứng  
- Có tables/migrations tương ứng (DB contract)  
- Có event in/out (nếu yêu cầu)  
- Có tests: unit/integration + **≥1 e2e**  
- Có seed/template tối thiểu (để “vào là xài được”)  
=> Module PASS khi tất cả Epic bắt buộc PASS.

#### 8.14.3 CI implementation (Cloud Build)
**Build steps (gợi ý)**
1) `pnpm i`  
2) `pnpm validate:openapi`  
3) `pnpm validate:capabilities` (readiness + capabilities)  
4) `pnpm test:unit` + `pnpm test:integration`  
5) `pnpm e2e:module-gates` (Playwright, tag-based)  
6) `pnpm release:coverage` (generate JSON+MD; attach links to reports)  
7) Upload artifacts: coverage + playwright report → Cloud Storage

> Cloud Build có hướng dẫn lưu artifacts lên Cloud Storage. citeturn0search0  
> Playwright HTML report mặc định output ra `playwright-report` và có thể override output dir. citeturn0search1turn0search14

#### 8.14.4 Hard gate: fail coverage → block release
- Nếu `coverage-report.json.status != "PASS"` → build fail.
- Nếu bất kỳ module ACTIVE (profile) có `status != PASS` → block promote.

---

### 8.15 Release Gate Dashboard (Admin) — nhìn 1 trang biết có thể go‑live hay chưa

> Dashboard này giúp bạn thấy **3 lớp readiness**:
1) **Build/Release coverage** (CI report) — “code đã đủ chưa”  
2) **Module readiness runtime** (`/system/module-health`) — “org đã cấu hình để dùng chưa”  
3) **Cost & Safety readiness** — “budget/safety gates OK chưa”

#### 8.15.1 UI screen
- Route: `/settings/release` (SuperAdmin only)
- Components:
  - `C-ReleaseGateDashboard` (summary)
  - `C-ModuleReadinessMatrix` (table per module)
  - links: Playwright HTML report, coverage-report.md, last commit/build id

#### 8.15.2 API contract (System/Release)
- `GET /system/release-gates/latest`  
  returns: `{ build_id, commit_sha, generated_at, profile, modules:[{key,status,missing[]}], links:{coverage_md, playwright_report} }`
- `POST /system/release-gates/report` (internal CI only)  
  body: `coverage-report.json` + signed artifact URLs
- `GET /system/module-health?org_id=`  
  returns runtime readiness: templates/entities/workflow runnable

> Playwright reports: “reporters” + output directory docs. citeturn0search1turn0search2

#### 8.15.3 Data model (ops/system)
- Table: `ops.release_gate_reports`
  - `id`, `environment`, `build_id`, `commit_sha`, `profile`, `status`, `report_json`, `links_json`, `created_at`
- RLS: only SuperAdmin (system scope) can read.

---

### 8.16 WP updates (Roadmap) — thêm track cho Coverage + Dashboard

- **WP-0.12: Coverage Report System (CI)**  
  - generate coverage JSON/MD, run manifest checks, publish artifacts, block on fail.
- **WP-6.6: Release Dashboard (Admin)**  
  - implement endpoints + table `ops.release_gate_reports` + UI `/settings/release`.

> Lưu artifacts trong Cloud Storage (Cloud Build): citeturn0search0
